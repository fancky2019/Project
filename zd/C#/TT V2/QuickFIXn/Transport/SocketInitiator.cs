...
namespace QuickFix.Transport
{
    ...
    public class SocketInitiator : AbstractInitiator
    {
        ...
        public static void SocketInitiatorThreadStart(object socketInitiatorThread)
        {
            ...
            {
                // Added by Rainer on 201501006 -begin
                try
                {
                    FixZDLogger zdLogger = ZDLoggerFactory.getSynWriteLogger("IssueCheck.log");
                    zdLogger.log(FixZDLogger.LVL_ERROR, ex.ToString());
                    zdLogger.log(FixZDLogger.LVL_ERROR, ex.StackTrace);
                }
                catch (Exception exi)
                {
                }
                // Added by Rainer on 201501006 -end
            ...
            catch (SocketException e)
            {
                ...
                {
                    FixZDLogger zdLogger = ZDLoggerFactory.getSynWriteLogger("IssueCheck.log");
                    zdLogger.log(FixZDLogger.LVL_ERROR, e.ToString());
                    zdLogger.log(FixZDLogger.LVL_ERROR, e.StackTrace);
                }
                catch (Exception exi)
                {
                }
            ...
            catch (Exception exe)
            {
                ...
                {
                    FixZDLogger zdLogger = ZDLoggerFactory.getSynWriteLogger("IssueCheck.log");
                    zdLogger.log(FixZDLogger.LVL_ERROR, exe.ToString());
                    zdLogger.log(FixZDLogger.LVL_ERROR, exe.StackTrace);
                }
                catch (Exception exi)
                {
                }

...
namespace QuickFix.Fields.Converters
{
    ...
    public static class DateTimeConverter
    {
        ...
        public const string pattern = @"\d{8}-\d{2}:\d{2}:\d{2}\.\d{6}";

        public const string pattern2 = @"\d{8}-\d{2}:\d{2}:\d{2}\.\d{3}";
        ...
        public static System.DateTime ConvertToDateTime(string str)
        {
            try
            {
                ...
                {
                    return DateTime.ParseExact(str, "yyyyMMdd-HH:mm:ss.ffffff", CultureInfo.InvariantCulture);
                }
                else if (Regex.IsMatch(str, pattern2)){
                    return DateTime.ParseExact(str, "yyyyMMdd-HH:mm:ss.fff", CultureInfo.InvariantCulture);
                }
                else

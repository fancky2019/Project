...
namespace QuickFix
{
    ...
    public class FileLog : ILog, System.IDisposable
    {
        ...
        public FileLog(string fileLogPath) : this(fileLogPath, "GLOBAL")
        {  }
        public FileLog(string fileLogPath, SessionID sessionID):this (fileLogPath, Prefix(sessionID)) { }
        
        public FileLog(string fileLogPath, string prefix)
        {
            this._fileLogPath = fileLogPath;
            this._prefix = prefix;
            Init(fileLogPath, prefix);
        }
        ...
        private void Init(string fileLogPath, string prefix)
        {
            ...
            messageLogFileName_ = System.IO.Path.Combine(fileLogPath, prefix + $".messages.current-{DateTime.Now.ToString("yyyy-MM-dd")}.log");
            eventLogFileName_ = System.IO.Path.Combine(fileLogPath, prefix + $".event.current-{DateTime.Now.ToString("yyyy - MM - dd")}.log");
        ...
        private void SameDayCheck()
        {
            if (DateTime.UtcNow.Date != _createTime.Date)
            {
                messageLog_.Close();
                eventLog_.Close();

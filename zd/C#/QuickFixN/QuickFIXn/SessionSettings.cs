...
namespace QuickFix
{
    ...
    public class SessionSettings
    {
        ...
        public const string SENDERCOMPPSW = "SenderCompPsw";
        ...
        protected void Load(TextReader conf)
        {
            ...
            foreach (QuickFix.Dictionary dict in section)
            {
                ...
                if (dict.Has(SENDERCOMPPSW))
                    senderCompPsw = dict.GetString(SENDERCOMPPSW);
                if (dict.Has(HEARTBTINT))
                    heartBtInt = dict.GetString(HEARTBTINT);

                
                SessionID sessionID = new SessionID(dict.GetString(BEGINSTRING), dict.GetString(SENDERCOMPID), senderSubID, senderLocID, dict.GetString(TARGETCOMPID), targetSubID, targetLocID, sessionQualifier, senderCompPsw, heartBtInt);
                Set(sessionID, dict);
            }
        }
